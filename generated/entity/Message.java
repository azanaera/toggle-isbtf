...
public class Message extends com.guidewire.pl.persistence.code.BeanBase implements entity.KeyableBean, gw.api.contact.ContactRetireBean {
  ...
  /**
   * This combines the behavior of reportError(Date retryTime) and reportError(ErrorCategory category).
   * 
   * If retryTime is null, it is equivalent to calling reportError(ErrorCategory category).
   * Otherwise, if category is null, it is equivalent to calling reportError(Date retryTime).
   * Otherwise, if retryTime is later than the current time, it sets the error category to the specified category
   * and schedules a retry at the specified time.
   * Otherwise, it throws an IllegalArgumentException.
   * @param retryTime the time later than now to retry the message.
   * @param category the error category of the message.
   */
  public void reportError(java.util.Date retryTime, typekey.ErrorCategory category) {
    ((com.guidewire.pl.domain.messaging.MessagePublicMethods)__getDelegateManager().getImplementation("com.guidewire.pl.domain.messaging.MessagePublicMethods")).reportError(retryTime, category);
  }
