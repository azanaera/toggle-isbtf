...
class EditCheckWizardExpressions {
  ...
  public static class EditCheckWizardExpressionsImpl extends gw.api.web.ScopeBaseClass {
    ...
    function beforeCommit_18 (pickedValue :  java.lang.Object) : void {
      wizard.CheckSet.RequestingUser = User.util.CurrentUser; gw.api.system.bundle.BundleCommitCondition.verify(claim); wizard.ensureNoCheckPayeesAreRetired(); wizard.performBeforeFinish()
    }
    ...
    function onExit_1 () : void {
      wizard.validateAllCheckPayees(); wizard.createNewCheckPortionsIfNecessary() ; ext.config.check.CheckScreensHelper_Ext.CheckAddressLinesLength(wizard.Check) ; wizard.refreshBundle(); if( !wizard.showPaymentsStep() ) { wizard.validatePayments() }
    }
