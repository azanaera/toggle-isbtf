...
class SearchCriteriaValidationPlugin implements ISearchCriteriaValidationPlugin {
  ...
  /**
   * Story card : Search or Create a Policy to create a NewClaim
   * The Policy number and Loss date are now mandatory for a policy search
   * @param searchCriteria
   * @return
   */
  override function validateCanSearch(searchCriteria: PolicySearchCriteria): String {
    return (searchCriteria.LossDate == null or not searchCriteria.PolicyNumber.HasContent)
            ? DisplayKey.get("Toggle.Java.SearchCriteria.Validation.PolicySearchCriteria_Ext")
            : null
  }
