...
uses ext.config.check.CheckScreensHelper_Ext#CheckLevyWithholding(Check)
...
enhancement GWCheckEnhancement : Check {
  ...
  public function reconstructPayTo() {
    if (this.PaymentMethod != PaymentMethod.TC_EFT) {
      ...
      this.Reportability = typekey.ReportabilityType.get((this.FirstPayee.PayeeType == TC_VENDOR) ? "reportable" : "notreportable")
    }
  }
  ...
  public function handleOnPickForPayee(checkPayee : CheckPayee) {
    ...
    reconstructPayTo()
    if (checkPayee != null){
      CheckLevyWithholding(checkPayee.Check)
    }
  }
