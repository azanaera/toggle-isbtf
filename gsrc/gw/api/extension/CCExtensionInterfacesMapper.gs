...
class CCExtensionInterfacesMapper implements ExtensionInterfacesMapper {
  ...
  override function bindImplementations(config: ExtensionInterfacesConfig) {
    ...
    /**
     * AB 5000 integration extensions
     */

    config.setImplementation<IContactIntegrationMapperFactoryExtensions_AB5000>(\-> gw.contactmapper.ab5000.ContactIntegrationMapperFactory.mapper())

    config.setImplementation<ICCNameMapperExtensions_AB5000>(\-> gw.contactmapper.ab5000.CCNameMapper.Instance)

    /**
     * AB 1000 integration extensions
     */

    config.setImplementation<IContactIntegrationMapperFactoryExtensions_AB1000>(\-> gw.contactmapper.ab1000.ContactIntegrationMapperFactory.mapper())

    config.setImplementation<ICCNameMapperExtensions_AB1000>(\-> gw.contactmapper.ab1000.CCNameMapper.Instance)
    ...
    config.setImplementation<IContactIntegrationMapperFactoryExtensions_AB900>(\-> gw.contactmapper.ab900.ContactIntegrationMapperFactory.mapper())

    config.setImplementation<ICCNameMapperExtensions_AB900>(\-> gw.contactmapper.ab900.CCNameMapper.Instance)
    ...
    config.setImplementation<ExtensionBinder<Claim>>(\ claim -> new IClaimExtensions() {
      ...
      override property get CoverageInQuestionDescription(): String {
        return claim.CoverageInQuestionDescription
      }
    ...
    config.setImplementation<ExtensionBinder<PolicyType>>(\ policyType -> new IPolicyTypeExtensions() {
      override function allowsRiskUnitSelection() : boolean {
        return policyType.AllowsRiskUnitSelection
      }
    })
    ...
    config.setImplementation<ExtensionBinder<MoneyMetricDelegate>>(\ metricDelegate -> new IMoneyMetricDelegateExtensions() {
      ...
      override property get ZeroDisplayValue(): String {
        return metricDelegate.ZeroDisplayValue
      }
    })
