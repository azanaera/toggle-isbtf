...
class ISORequestBase {
  ...
  protected function createSignOnRequest(acord : xsd.iso.req.ACORD) {
    try {
      ...
      var cred = plugin.retrieveUsernameAndPassword("iso")
      acord.SignonRq.SignonPswd.CustId.SPName = Properties.SPName
      acord.SignonRq.SignonPswd.CustId.CustPermId = cred.getUsername()
      acord.SignonRq.SignonPswd.CustId.CustLoginId = cred.getUsername()
      acord.SignonRq.SignonPswd.CustPswd.EncryptionTypeCd = Properties.EncryptionTypeCode
      acord.SignonRq.SignonPswd.CustPswd.Pswd = cred.getPassword()
    }catch(e : NullPointerException){
      _logger.error("Exception retrieving credentials", #createSignOnRequest(xsd.iso.req.ACORD), e )
