...
class ISOReplyBase implements IISOReply {
  ...
  function updateReportableOnSuccess() {
    ...
    if (Reportable.ISOMatchReports.hasMatch(\ m -> m.New)) {
      var path = "ClaimInvestigationAddRs/MatchReportTypeCd"
      var matchReportTypeCdContent = _isoResponse.getElementContent(path)
      if (matchReportTypeCdContent.equals("V")){
        ...
        activity.Subject = DisplayKey.get("Toggle.ISO.Activity.VehicleWasRecovered.Subject_Ext")
        activity.Description =DisplayKey.get("Toggle.ISO.Activity.VehicleWasRecovered.Discription_Ext")
      }
    }
