...
class ISOAutoLossSectionBase extends ISOLossSection {
  ...
  /**
   * Override of the createAndAddInvestigationInfo method to add in the additional
   * com.iso_TheftTypeInd fields to the InvestigationInfo
   * occurrence. Returns the investigationInfo object.
   */
  public override function createAndAddInvestigationInfo() : InvestigationInfo {
    var investigationInfo = super.createAndAddInvestigationInfo()
    if ((investigationInfo == null) and shouldAddExtraFieldsToInvestigationInfo()){
      investigationInfo = new InvestigationInfo()
    }
    var losscause = SearchRequest.Claim.LossCause
    if(LossCause.TC_THEFTENTIRE.equals(losscause)){
      investigationInfo.ComIso_TheftTypeInd ="T"
    }
    if(LossCause.TC_THEFTPARTS.equals(losscause)) {
      investigationInfo.ComIso_TheftTypeInd ="P"
    }

    if(shouldAddExtraFieldsToInvestigationInfo()){
  ...
  /**
   * Called by createAndAddInvestigationInfo() to decide whether to add an AdditionalField
   * i.e."com.iso_TheftTypeInd" (TotalLoss or partial loss)
   * to InvestigationInfo. Returns false by default.
   */
  protected function shouldAddExtraFieldsToInvestigationInfo() : boolean {
    if((LossCause.TC_THEFTENTIRE.equals(SearchRequest.Claim.LossCause)) or (LossCause.TC_THEFTPARTS.equals(SearchRequest.Claim.LossCause))){
      return true
    }
    return false
  }
